    ...
    static class Program
    {
        ...
        static void doYurixyAssociation()
        {
            ...
            foreach (string regEntry in getRidOf)
            {
                try
                {
                    Microsoft.Win32.Registry.ClassesRoot.DeleteSubKeyTree(regEntry);
                }
                catch// (Exception ex)
                {
                    //debugadd("Got expected error trying to delete reg key");
            ...
            {
                //try to register my key
                Microsoft.Win32.RegistryKey filekey;
                //Microsoft.Win32.RegistryKey dikey;
                ...
                Microsoft.Win32.RegistryKey appDefaultKey = appkey.CreateSubKey("DefaultIcon");
                appDefaultKey.SetValue("", iconPath);

                Microsoft.Win32.RegistryKey commandKey = appkey.CreateSubKey("shell\\open\\command");
                commandKey.SetValue("", "\"" + Application.StartupPath + "\\Skin Installer Ultimate.exe" + "\" --file \"%1\"");
                appDefaultKey.Close();
                commandKey.Close();
                appkey.Close();
            }
