...
namespace LOLViewer
{
    class GLTexture
    {
        ...
        private bool CreateDDSTexture(RAFFileListEntry f, TextureTarget target)
        {
            ...
            {
                return CreateDDSTexture(new FileInfo(f.FileName), target);
            }
            ...
            try
            {
                //check for : then dont do this!
                RAFArchive raf = new RAFArchive(f.RAFArchive.RAFFilePath);
                TextureLoaders.ImageDDS.LoadFromMemory(
                    raf.GetDirectoryFile().GetFileList().GetFileEntry(f.FileName).GetContent(),
                    out handle,
                    out dimension);
